...
//! the [`AstralisRpc`] API. CLI clients connect via `WebSocket`.
//!
//! # Locking Design
//!
...
use astralis_approval::budget::{WorkspaceBudgetSnapshot, WorkspaceBudgetTracker};
...
pub struct DaemonPaths {
    /// Directory for daemon files (e.g. `~/.astralis/`).
    pub base_dir: PathBuf,
}

impl DaemonPaths {
    /// Create paths for the default location using `AstralisHome`.
    ///
    /// # Errors
    ///
...
impl DaemonServer {
    ...
    pub async fn start(
        ...
        let ws = astralis_core::dirs::WorkspaceDir::detect(&cwd);
        ...
        let cfg = match astralis_config::Config::load(Some(&cwd)) {
            Ok(r) => r.config,
            Err(e) => {
                warn!(error = %e, "Failed to load config; falling back to defaults");
                astralis_config::Config::default()
            },
        };
        ...
        // Lives in ~/.astralis/state.db, namespaced by workspace ID.
...
impl AstralisRpcServer for RpcImpl {
        ...
        Ok(entries[start..]
            ...
            .map(|entry| AuditEntryInfo {
                ...
                outcome: match &entry.outcome {
                    astralis_audit::AuditOutcome::Success { details } => {
                    ...
                    astralis_audit::AuditOutcome::Failure { error } => {
                        format!("FAIL: {error}")
                    },
                },
            })
...
impl RpcImpl {
    ...
    async fn load_workspace_escape(&self) -> Option<astralis_workspace::escape::EscapeState> {
    ...
    async fn save_workspace_escape(&self, state: &astralis_workspace::escape::EscapeState) {
        let ns = ws_ns(&self.workspace_id, "escape");
