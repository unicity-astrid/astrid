...
use astralis_core::{ElicitationRequest, ElicitationResponse, ElicitationSchema};
...
impl ElicitationManager {
    ...
    async fn send_select(
        ...
        request_id: &str,
        header: &str,
        options: &[astralis_core::SelectOption],
    ) {
...
mod tests {
    ...
    fn build_response_cancel() {
        ...
        assert!(matches!(
            resp.action,
            astralis_core::ElicitationAction::Cancel
        ));
    }
    ...
    fn build_response_yes() {
        ...
        if let astralis_core::ElicitationAction::Submit { value } = &resp.action {
