        ...
        Self::new("logging", "Profile that logs all major events to stdout")
            .with_hook(
                Hook::new(HookEvent::SessionStart)
                    ...
                    .with_handler(HookHandler::Command {
                        ...
                        args: vec!["[ASTRALIS] Session started: $ASTRALIS_SESSION_ID".to_string()],
            ...
            .with_hook(
                Hook::new(HookEvent::SessionEnd)
                    ...
                    .with_handler(HookHandler::Command {
                        command: "echo".to_string(),
                        args: vec!["[ASTRALIS] Session ended: $ASTRALIS_SESSION_ID".to_string()],
                        env: std::collections::HashMap::new(),
                        working_dir: None,
                    })
                    .with_fail_action(FailAction::Ignore)
            ...
            .with_hook(
                Hook::new(HookEvent::PreToolCall)
                    ...
                    .with_handler(HookHandler::Command {
                        command: "echo".to_string(),
                        args: vec!["[ASTRALIS] Tool call: $ASTRALIS_HOOK_DATA".to_string()],
                        env: std::collections::HashMap::new(),
                        working_dir: None,
                    })
                    .with_fail_action(FailAction::Ignore)
            ...
            Hook::new(HookEvent::PreToolCall)
                ...
                .with_handler(HookHandler::Command {
                    ...
                    args: vec![
                        "-c".to_string(),
                        r#"
                        TOOL=$(echo "$ASTRALIS_HOOK_DATA" | grep -o '"tool_name":"[^"]*"' | cut -d'"' -f4)
                        case "$TOOL" in
        ...
        .with_hook(
            Hook::new(HookEvent::PreToolCall)
                ...
                .with_handler(HookHandler::Command {
                    ...
                    args: vec![
                        "-c".to_string(),
                        r#"echo "[DEBUG] Tool call at $(date): $ASTRALIS_HOOK_DATA" >> /tmp/astralis-debug.log"#.to_string(),
                    ],
                    env: std::collections::HashMap::new(),
                    working_dir: None,
                })
        ...
        .with_hook(
            Hook::new(HookEvent::ToolError)
                ...
                .with_handler(HookHandler::Command {
                    ...
                    args: vec![
                        "-c".to_string(),
                        r#"echo "[ERROR] Tool error at $(date): $ASTRALIS_HOOK_DATA" >> /tmp/astralis-debug.log"#.to_string(),
                    ],
                    env: std::collections::HashMap::new(),
                    working_dir: None,
                })
