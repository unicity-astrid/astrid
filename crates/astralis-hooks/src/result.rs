...
impl HookContext {
    ...
    pub fn to_env_vars(&self) -> HashMap<String, String> {
        ...
        env.insert(
            "ASTRALIS_HOOK_ID".to_string(),
            self.invocation_id.to_string(),
        );
        env.insert("ASTRALIS_HOOK_EVENT".to_string(), self.event.to_string());
        env.insert(
            "ASTRALIS_HOOK_TIMESTAMP".to_string(),
            self.timestamp.to_rfc3339(),
        );

        if let Some(session_id) = &self.session_id {
            env.insert("ASTRALIS_SESSION_ID".to_string(), session_id.to_string());
        }

        if let Some(user_id) = &self.user_id {
            env.insert("ASTRALIS_USER_ID".to_string(), user_id.to_string());
        }
        ...
        {
            env.insert("ASTRALIS_HOOK_DATA".to_string(), json);
        }
...
mod tests {
    ...
    fn test_hook_context_env_vars() {
        ...
        assert!(env.contains_key("ASTRALIS_HOOK_ID"));
        assert_eq!(
            env.get("ASTRALIS_HOOK_EVENT"),
