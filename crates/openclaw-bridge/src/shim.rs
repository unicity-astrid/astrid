...
function hostLog(level, msg) {{
  var lm = _toMem(level);
  var mm = _toMem(msg);
  _getHostFn("astralis_log")(lm.offset, mm.offset);
}}

function hostGetConfig(key) {{
  var km = _toMem(key);
  var result = _getHostFn("astralis_get_config")(km.offset);
  return _fromMem(result);
}}

function hostReadFile(path) {{
  var pm = _toMem(path);
  var result = _getHostFn("astralis_read_file")(pm.offset);
  return _fromMem(result);
}}
...
function hostWriteFile(path, content) {{
  ...
  _getHostFn("astralis_write_file")(pm.offset, cm.offset);
}}

function hostKvGet(key) {{
  var km = _toMem(key);
  var result = _getHostFn("astralis_kv_get")(km.offset);
  return _fromMem(result);
}}
...
function hostKvSet(key, value) {{
  var km = _toMem(key);
  var vm = _toMem(value);
  _getHostFn("astralis_kv_set")(km.offset, vm.offset);
}}

function hostHttpRequest(requestJson) {{
  var rm = _toMem(requestJson);
  var result = _getHostFn("astralis_http_request")(rm.offset);
  return _fromMem(result);
}}
...
Host.__hostFunctions = [
  {{ name: "astralis_get_config", results: 1 }},
  {{ name: "astralis_http_request", results: 1 }},
  {{ name: "astralis_kv_get", results: 1 }},
  {{ name: "astralis_kv_set", results: 0 }},
  {{ name: "astralis_log", results: 0 }},
  {{ name: "astralis_read_file", results: 1 }},
  {{ name: "astralis_write_file", results: 0 }}
];
