...
pub(crate) fn pack_capsule_archive(
    ...
    if let Some(wasm) = wasm_path {
        if wasm.exists() {
            let mut wasm_file = File::open(wasm)
                .with_context(|| format!("Failed to open WASM binary for packing: {}", wasm.display()))?;
            let file_name = wasm.file_name().unwrap_or_default();
            tar.append_file(file_name, &mut wasm_file)
                .with_context(|| format!("Failed to append WASM binary to archive: {}", wasm.display()))?;
        } else {
    ...
    for file_path in additional_files {
        if file_path.exists() {
            let rel_path = file_path.strip_prefix(base_dir).unwrap_or(Path::new(file_path.file_name().unwrap_or_default()));
            
            if file_path.is_dir() {
                tar.append_dir_all(rel_path, file_path)
                    .with_context(|| format!("Failed to append directory to archive: {}", file_path.display()))?;
            } else {
                let mut f = File::open(file_path)
                    .with_context(|| format!("Failed to open file for packing: {}", file_path.display()))?;
                tar.append_file(rel_path, &mut f)
                    .with_context(|| format!("Failed to append file to archive: {}", file_path.display()))?;
            }
        }
    }
