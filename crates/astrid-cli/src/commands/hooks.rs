...
pub(crate) fn list_hooks() {
    ...
    let hooks_dir =
        AstridHome::resolve().map_or_else(|_| PathBuf::from(".astrid/hooks"), |h| h.hooks_dir());
...
pub(crate) fn hook_info(name: &str) {
    ...
    let extra_paths: Vec<PathBuf> = AstridHome::resolve()
...
pub(crate) fn list_profiles() {
    ...
    println!("\nApply with: astrid hooks apply-profile <name>");
}
