...
LLM provider abstraction with streaming support for Astralis.
...
```rust
use astralis_llm::{ClaudeProvider, LlmProvider, ProviderConfig};
...
```rust
use astralis_llm::{OpenAiCompatProvider, LlmProvider};
...
```rust
use astralis_llm::{ClaudeProvider, LlmProvider, Message, StreamEvent};
