...
use astralis_approval::allowance::Allowance;
use astralis_approval::budget::{BudgetSnapshot, BudgetTracker, WorkspaceBudgetTracker};
use astralis_approval::{AllowanceStore, ApprovalManager, DeferredResolutionStore};
use astralis_capabilities::CapabilityStore;
use astralis_core::SessionId;
use astralis_llm::Message;
use astralis_workspace::escape::{EscapeHandler, EscapeState};
...
impl AgentSession {
    ...
    pub fn import_workspace_allowances(
        &self,
        allowances: Vec<astralis_approval::allowance::Allowance>,
    ) {
    ...
    #[must_use]
    pub fn export_workspace_allowances(&self) -> Vec<astralis_approval::allowance::Allowance> {
    ...
    pub async fn with_persistent_deferred_queue(
        mut self,
        store: astralis_storage::ScopedKvStore,
    ) -> Result<Self, astralis_core::error::SecurityError> {
        ...
        Self {
            ...
            messages: session
                ...
                .map(|m| SerializableMessage {
                    role: match m.role {
                        astralis_llm::MessageRole::System => "system".to_string(),
                        astralis_llm::MessageRole::User => "user".to_string(),
                        astralis_llm::MessageRole::Assistant => "assistant".to_string(),
                        astralis_llm::MessageRole::Tool => "tool".to_string(),
                    },
...
impl SerializableSession {
    ...
    pub fn to_session(&self) -> AgentSession {
        ...
        let messages: Vec<Message> = self
            ...
            .iter()
            .filter_map(|m| {
                let content: astralis_llm::MessageContent =
                    serde_json::from_value(m.content.clone()).ok()?;
                let role = match m.role.as_str() {
                    "system" => astralis_llm::MessageRole::System,
                    "user" => astralis_llm::MessageRole::User,
                    "assistant" => astralis_llm::MessageRole::Assistant,
                    "tool" => astralis_llm::MessageRole::Tool,
                    _ => return None,
                };
