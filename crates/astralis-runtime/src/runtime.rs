...
use astralis_approval::manager::ApprovalHandler;
...
use astralis_approval::{SecurityInterceptor, SecurityPolicy, SensitiveAction};
...
use astralis_hooks::result::HookContext;
use astralis_hooks::{HookEvent, HookManager};
use astralis_llm::{
    LlmProvider, LlmToolDefinition, Message, StreamEvent, ToolCall, ToolCallResult,
};
use astralis_mcp::McpClient;
use astralis_tools::{ToolContext, ToolRegistry, truncate_output};
use astralis_workspace::{
...
impl<P: LlmProvider + 'static> AgentRuntime<P> {
    ...
    pub fn create_session(&self, workspace_override: Option<&Path>) -> AgentSession {
        ...
        let system_prompt = if self.config.system_prompt.is_empty() {
            astralis_tools::build_system_prompt(workspace_root)
        } else {
    ...
    pub async fn run_turn_streaming<F: Frontend + 'static>(
        ...
        // Fire UserPrompt hook
        {
            ...
            if let astralis_hooks::HookResult::Block { reason } = result {
                return Err(RuntimeError::ApprovalDenied { reason });
            }
            if let astralis_hooks::HookResult::ContinueWith { modifications } = &result {
                ...
                match event? {
                    ...
                    StreamEvent::Error(e) => {
                        ...
                        return Err(RuntimeError::LlmError(
                            astralis_llm::LlmError::StreamingError(e),
                        ));
                    },
                }
            }
    ...
    async fn execute_tool_call<F: Frontend>(
        ...
        let (server, tool) = call.parse_name().ok_or_else(|| {
            RuntimeError::McpError(astralis_mcp::McpError::ToolNotFound {
                            ...
                            AuditAction::ApprovalGranted {
                                ...
                                scope: match decision.decision {
                                    ApprovalOption::AllowSession => {
                                        astralis_audit::ApprovalScope::Session
                                    },
                                    ApprovalOption::AllowWorkspace => {
                                        astralis_audit::ApprovalScope::Workspace
                                    },
                                    ApprovalOption::AllowAlways => {
                                        astralis_audit::ApprovalScope::Always
                                    },
                                    ApprovalOption::AllowOnce | ApprovalOption::Deny => {
                                        astralis_audit::ApprovalScope::Once
                                    },
                                },
                            },
...
fn infer_operation(tool: &str) -> astralis_workspace::escape::EscapeOperation {
    use astralis_workspace::escape::EscapeOperation;
...
fn risk_level_for_operation(operation: astralis_workspace::escape::EscapeOperation) -> RiskLevel {
    use astralis_workspace::escape::EscapeOperation;
    match operation {
