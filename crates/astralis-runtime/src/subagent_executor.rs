...
use astralis_audit::{AuditAction, AuditOutcome, AuthorizationProof};
use astralis_core::{Frontend, SessionId};
use astralis_llm::{LlmProvider, Message, MessageContent, MessageRole};
use astralis_tools::{SubAgentRequest, SubAgentResult, SubAgentSpawner};
...
pub struct SubAgentExecutor<P: LlmProvider, F: Frontend + 'static> {
    ...
    parent_allowance_store: Arc<astralis_approval::AllowanceStore>,
    /// Parent's capability store (shared with child for capability inheritance).
    parent_capabilities: Arc<astralis_capabilities::CapabilityStore>,
    /// Parent's budget tracker (shared with child so spend is visible bidirectionally).
    parent_budget_tracker: Arc<astralis_approval::budget::BudgetTracker>,
