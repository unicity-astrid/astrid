...
//! 2. Merge `/etc/astralis/config.toml` (system)
//! 3. Merge `~/.astralis/config.toml` (user)
//! 4. Merge `{workspace}/.astralis/config.toml` (workspace) + restriction enforcement
...
pub fn load(workspace_root: Option<&Path>) -> ConfigResult<ResolvedConfig> {
    ...
    // 2. System config (/etc/astralis/config.toml).
    let system_path = PathBuf::from("/etc/astralis/config.toml");
    ...
    // 3. User config (~/.astralis/config.toml).
    let user_path = home_dir.join(".astralis").join("config.toml");
    if let Some(overlay) = try_load_file(&user_path)? {
    ...
    } else if let Some(astralis_home) = env_vars.get("ASTRALIS_HOME") {
        // ASTRALIS_HOME provides an alternate user-level config directory.
        ...
        // This prevents an attacker from setting ASTRALIS_HOME to a directory
        // they control to inject config at the trusted User level.
        let validated = validate_astralis_home(astralis_home, &home_dir);
        ...
        if let Some(canonical) = validated {
            ...
            if let Some(overlay) = try_load_file(&alt_path)? {
                ...
                info!(path = %alt_path.display(), "loaded user config from ASTRALIS_HOME");
            }
        } else {
            tracing::warn!(
                path = astralis_home,
                "ASTRALIS_HOME is not a valid directory owned by current user; ignoring"
            );
        }
    }

    // 4. Workspace config ({workspace}/.astralis/config.toml).
    ...
    if let Some(ws_root) = workspace_root {
        let ws_path = ws_root.join(".astralis").join("config.toml");
        if let Some(mut overlay) = try_load_file(&ws_path)? {
            // Resolve ${VAR} references in workspace overlay with restricted
            // env vars (only ASTRALIS_* and ANTHROPIC_*). This prevents a
...
/// Validate that an `ASTRALIS_HOME` path is a real directory owned by the
/// same user who owns `home_dir`. Returns the canonicalized path on success.
fn validate_astralis_home(raw_path: &str, home_dir: &Path) -> Option<PathBuf> {
