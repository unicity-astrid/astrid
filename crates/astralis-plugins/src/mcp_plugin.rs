...
//! [`SecurityInterceptor`](astralis_approval::SecurityInterceptor) checks
...
//! (`notifications/astralis.hookEvent`) over the MCP connection.
...
use astralis_core::HookEvent;
use astralis_mcp::{AstralisClientHandler, CapabilitiesHandler, McpClient, ToolResult};
...
type PluginMcpService = RunningService<RoleClient, AstralisClientHandler>;
...
impl McpPlugin {
    ...
    /// `notifications/astralis.hookEvent`. This is fire-and-forget;
    ...
    pub async fn send_hook_event(&self, event: HookEvent, data: Value) {
        ...
        let notification = CustomNotification::new(
            "notifications/astralis.hookEvent",
    ...
    fn verify_binary_hash(&self) -> PluginResult<()> {
        ...
        let actual_hash = astralis_crypto::ContentHash::hash(&binary_data);
...
impl Plugin for McpPlugin {
    ...
    async fn load(&mut self, _ctx: &PluginContext) -> PluginResult<()> {
        ...
        let client_handler = AstralisClientHandler::new(&self.server_name, handler);
...
mod tests {
    ...
    fn test_mcp_client() -> McpClient {
        McpClient::with_config(astralis_mcp::ServersConfig::default())
    }
