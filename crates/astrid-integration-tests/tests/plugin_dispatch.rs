...
async fn test_security_interceptor_denial() {
    ...
    // Build a policy that blocks the plugin tool.
    // classify_tool_call produces McpToolCall { server: "plugin:test", tool: "echo" }
    // so the policy checks "plugin:test:echo", "plugin:test", and "echo".
    let mut policy = astrid_approval::SecurityPolicy::default();
    policy.blocked_tools.insert("plugin:test:echo".to_string());
