...
async fn test_wasm_capsule_e2e_env_config_injection() {
    ...
    env.insert("test_key".into(), astrid_capsule::manifest::EnvDef {
        env_type: "string".into(),
        default: Some(json!("default_value")),
        request: None,
        description: None,
    });
    env.insert("injected_key".into(), astrid_capsule::manifest::EnvDef {
    ...
    let mut capsule = loader.create_capsule(manifest, fixture_path.parent().unwrap().to_path_buf()).unwrap();
    ...
    kv.set("injected_key", b"injected_value".to_vec()).await.unwrap();
    ...
    let config_tool = tools_list.iter().find(|t| t.name() == "test-config").unwrap();

    // 1. Read default value
    let res1 = config_tool.execute(json!({ "key": "test_key" }), &tool_ctx).await.unwrap();
    let out1: serde_json::Value = serde_json::from_str(&res1).unwrap();
    assert_eq!(out1["found"], true);
    assert_eq!(out1["value"], "default_value");

    // 2. Read injected KV value
    let res2 = config_tool.execute(json!({ "key": "injected_key" }), &tool_ctx).await.unwrap();
    let out2: serde_json::Value = serde_json::from_str(&res2).unwrap();
    assert_eq!(out2["found"], true);
    assert_eq!(out2["value"], "injected_value");

    // 3. Read missing value
    let res3 = config_tool.execute(json!({ "key": "missing_key" }), &tool_ctx).await.unwrap();
    let out3: serde_json::Value = serde_json::from_str(&res3).unwrap();
