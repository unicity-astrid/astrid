...
/// A client that connects to the Astralis daemon.
...
impl DaemonClient {
    ...
    ///
    /// Reads the port from `~/.astralis/daemon.port`. If the daemon isn't running,
    ...
    ///
    /// Stderr is redirected to `~/.astralis/logs/daemon.log` so startup errors
    /// can be surfaced to the user if the daemon fails to come up.
    async fn start_daemon(paths: &DaemonPaths) -> anyhow::Result<()> {
        // Find the current executable — the daemon runs as `astralis daemon run`.
