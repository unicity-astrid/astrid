...
pub(crate) async fn run_gateway(foreground: bool, config_path: Option<&str>) -> Result<()> {
    println!("{}", "Starting Astralis Gateway...".cyan().bold());
    ...
    let config = if let Some(path) = config_path {
        ...
        let unified = astralis_config::Config::load(cwd.as_deref())
            .map(|r| r.config)
            .unwrap_or_default();
        astralis_gateway::config_bridge::from_unified_config(&unified)
    };
