...
use anyhow::Result;
use astralis_gateway::{GatewayConfig, GatewayRuntime, HealthState};
use colored::Colorize;

/// Run health checks against the gateway.
pub(crate) async fn run_doctor() -> Result<()> {
    println!("{}", "Astralis Doctor - System Health Check".cyan().bold());
    println!();
    ...
    match astralis_mcp::ServersConfig::load_default() {
        Ok(config) => {
    ...
    if status.state == HealthState::Healthy {
        println!("{}", "Astralis is ready to use!".green().bold());
    } else {
        println!(
            "{}",
            "Please address the issues above before using Astralis."
                .yellow()
                .bold()
        );
    }
