...
pub(crate) fn run_onboarding() {
    ...
    println!(
        "{}",
        "  Welcome to Astralis! Let's configure your LLM provider.".bold()
    );
    println!();
...
pub(crate) fn has_api_key() -> bool {
    ...
    if env_is_set("ASTRALIS_MODEL_API_KEY") {
        return true;
    }
...
fn prompt_and_save_cloud(provider: &ProviderInfo) {
    ...
    println!();
    println!(
        "  {} {} configured and saved to ~/.astralis/config.toml",
        "✓".green().bold(),
        provider.label
    );
    println!();
...
/// section of `~/.astralis/config.toml`.
...
fn print_manual_instructions() {
    ...
    println!();
    println!("  Edit {}:", "~/.astralis/config.toml".cyan());
    println!();
...
/// Resolve the path to `~/.astralis/config.toml`.
fn global_config_path() -> Option<PathBuf> {
    astralis_core::dirs::AstralisHome::resolve()
        .ok()
        .map(|h| h.config_path())
}
