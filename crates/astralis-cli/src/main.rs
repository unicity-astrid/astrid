...
//! Astralis CLI - Secure Agent Runtime
//!
...
/// Astralis - Secure Agent Runtime
#[derive(Parser)]
#[command(name = "astralis")]
...
fn ensure_global_config() -> bool {
    let Ok(home) = astralis_core::dirs::AstralisHome::resolve() else {
        return false;
    };

    if let Err(e) = home.ensure() {
        eprintln!("Warning: could not create ~/.astralis directory: {e}");
        return false;
    }
    ...
    let template = r#"# Astralis configuration
# Documentation: https://github.com/astralis-rs/astralis
#
...
async fn main() -> Result<()> {
    ...
    let unified_cfg = astralis_config::Config::load(workspace_root.as_deref())
        .ok()
        .map(|r| r.config);
    ...
    let log_config = if let Some(cfg) = &unified_cfg {
        ...
        astralis_telemetry::LogConfig::new(level)
...
fn resolve_session_id(
    store: &astralis_runtime::SessionStore,
    id: Option<String>,
    last: bool,
) -> Result<String> {
    match (id, last) {
    ...
    match command {
        ServerCommands::List => {
            ...
            if let Ok(dc) = crate::daemon_client::DaemonClient::connect().await {
                ...
                let config = astralis_mcp::ServersConfig::load_default().unwrap_or_default();
