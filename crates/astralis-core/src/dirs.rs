//! Directory scaffolding for Astralis home and workspace directories.
//!
//! Two key directory structures:
//!
//! - [`AstralisHome`]: Global state at `~/.astralis/` (or `$ASTRALIS_HOME`).
...
//!
//! - [`WorkspaceDir`]: Per-project directory at `<project>/.astralis/`.
...
//! └── ASTRALIS.md                   (project-level instructions)
//! ```
...
/// Global Astralis home directory (`~/.astralis/` or `$ASTRALIS_HOME`).
///
/// Contains keys, audit databases, capability stores, and global config.
#[derive(Debug, Clone)]
pub struct AstralisHome {
    root: PathBuf,
}
...
impl AstralisHome {
    ...
    ///
    /// Checks `$ASTRALIS_HOME` first, then falls back to `$HOME/.astralis/`.
    ///
    /// # Errors
    ///
    /// Returns an error if neither `$ASTRALIS_HOME` nor `$HOME` is set.
    pub fn resolve() -> io::Result<Self> {
        let root = if let Ok(custom) = std::env::var("ASTRALIS_HOME") {
            let p = PathBuf::from(&custom);
            if !p.is_absolute() {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidInput,
                    "ASTRALIS_HOME must be an absolute path",
                ));
            }
            p
        } else {
            let home = std::env::var("HOME").map_err(|_| {
                io::Error::new(
                    ...
                    "neither ASTRALIS_HOME nor HOME environment variable is set",
                )
            })?;
            PathBuf::from(home).join(".astralis")
        };
    ...
    /// Keys directory (`~/.astralis/keys/`).
    #[must_use]
    pub fn keys_dir(&self) -> PathBuf {
        self.root.join("keys")
    }

    /// Logs directory (`~/.astralis/logs/`).
    #[must_use]
    ...
    /// Sessions directory (`~/.astralis/sessions/`).
    #[must_use]
...
/// Per-project workspace directory (`<project>/.astralis/`).
///
/// Contains only committable project-level config. A `workspace-id` UUID
/// links the project to its global state in `~/.astralis/`.
#[derive(Debug, Clone)]
pub struct WorkspaceDir {
    /// The project root (parent of `.astralis/`).
...
impl WorkspaceDir {
    ...
    ///
    /// Detection order:
    /// 1. Directory containing `.astralis/`
    /// 2. Directory containing `.git`
    /// 3. Directory containing `ASTRALIS.md`
    ...
    pub fn detect(start_dir: &Path) -> Self {
        ...
        loop {
            // Check for .astralis/ directory
            if current.join(".astralis").is_dir() {
                return Self {
            ...
            // Check for ASTRALIS.md
            if current.join("ASTRALIS.md").exists() {
                return Self {
                    project_root: current.to_path_buf(),
                };
            }
    ...
    /// Ensure the `.astralis/` directory exists and generate a workspace ID
    ...
    pub fn ensure(&self) -> io::Result<()> {
        std::fs::create_dir_all(self.dot_astralis())?;
        // Generate workspace ID if missing (idempotent — reads existing).
        let _ = self.workspace_id()?;
        Ok(())
    }

    /// Project root directory (parent of `.astralis/`).
    #[must_use]
    pub fn root(&self) -> &Path {
        &self.project_root
    }

    /// The `.astralis/` directory itself.
    #[must_use]
    pub fn dot_astralis(&self) -> PathBuf {
        self.project_root.join(".astralis")
    }

    /// Path to the workspace-id file (`.astralis/workspace-id`).
    #[must_use]
    pub fn workspace_id_path(&self) -> PathBuf {
        self.dot_astralis().join("workspace-id")
    }
    ...
    /// Path to the project-level instructions file (`.astralis/ASTRALIS.md`).
    #[must_use]
    pub fn instructions_path(&self) -> PathBuf {
        self.dot_astralis().join("ASTRALIS.md")
    }
}
...
mod tests {
    ...
    /// Mutex to serialize tests that mutate the `ASTRALIS_HOME` env var.
    /// `set_var`/`remove_var` are process-wide and unsafe under concurrency.
    static ENV_MUTEX: Mutex<()> = Mutex::new(());

    #[test]
    fn test_astralis_home_resolve_with_env() {
        let _guard = ENV_MUTEX.lock().unwrap();
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().to_path_buf();

        // SAFETY: serialized by ENV_MUTEX
        unsafe { std::env::set_var("ASTRALIS_HOME", &path) };
        let home = AstralisHome::resolve().unwrap();
        assert_eq!(home.root(), path);
        unsafe { std::env::remove_var("ASTRALIS_HOME") };
    }

    #[test]
    fn test_astralis_home_resolve_default() {
        let _guard = ENV_MUTEX.lock().unwrap();
        // SAFETY: serialized by ENV_MUTEX
        unsafe { std::env::remove_var("ASTRALIS_HOME") };
        let home = AstralisHome::resolve().unwrap();
        let expected = PathBuf::from(std::env::var("HOME").unwrap()).join(".astralis");
        assert_eq!(home.root(), expected);
    }

    #[test]
    fn test_astralis_home_ensure_creates_dirs() {
        let dir = tempfile::tempdir().unwrap();
        let home = AstralisHome::from_path(dir.path());
        home.ensure().unwrap();
    ...
    #[cfg(unix)]
    #[test]
    fn test_astralis_home_ensure_sets_permissions() {
        use std::os::unix::fs::PermissionsExt;
    ...
    #[test]
    fn test_astralis_home_rejects_relative_env() {
        let _guard = ENV_MUTEX.lock().unwrap();
        // SAFETY: serialized by ENV_MUTEX
        unsafe { std::env::set_var("ASTRALIS_HOME", "relative/path") };
        let result = AstralisHome::resolve();
    ...
    #[test]
    fn test_astralis_home_rejects_empty_env() {
        let _guard = ENV_MUTEX.lock().unwrap();
        // SAFETY: serialized by ENV_MUTEX
        unsafe { std::env::set_var("ASTRALIS_HOME", "") };
        let result = AstralisHome::resolve();
        assert!(result.is_err());
        unsafe { std::env::remove_var("ASTRALIS_HOME") };
    }
    ...
    #[test]
    fn test_astralis_home_path_accessors() {
        let home = AstralisHome::from_path("/tmp/test-astralis");
        assert_eq!(home.root(), Path::new("/tmp/test-astralis"));
        assert_eq!(home.keys_dir(), PathBuf::from("/tmp/test-astralis/keys"));
        assert_eq!(home.logs_dir(), PathBuf::from("/tmp/test-astralis/logs"));
        assert_eq!(
            home.user_key_path(),
            PathBuf::from("/tmp/test-astralis/keys/user.key")
        );
        assert_eq!(
            home.audit_db_path(),
            PathBuf::from("/tmp/test-astralis/audit.db")
        );
        assert_eq!(
            home.capabilities_db_path(),
            PathBuf::from("/tmp/test-astralis/capabilities.db")
        );
        assert_eq!(
            home.deferred_db_path(),
            PathBuf::from("/tmp/test-astralis/deferred.db")
        );
        assert_eq!(
            home.servers_config_path(),
            PathBuf::from("/tmp/test-astralis/servers.toml")
        );
        assert_eq!(
            home.config_path(),
            PathBuf::from("/tmp/test-astralis/config.toml")
        );
        assert_eq!(
            home.sessions_dir(),
            PathBuf::from("/tmp/test-astralis/sessions")
        );
        assert_eq!(
            home.state_db_path(),
    ...
    #[test]
    fn test_workspace_detect_with_dot_astralis() {
        let dir = tempfile::tempdir().unwrap();
        let astralis_dir = dir.path().join(".astralis");
        std::fs::create_dir(&astralis_dir).unwrap();
    ...
    #[test]
    fn test_workspace_detect_with_astralis_md() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(dir.path().join("ASTRALIS.md"), "# Project").unwrap();
    ...
    fn test_workspace_detect_fallback() {
        ...
        // No .astralis, .git, or ASTRALIS.md anywhere in the tree up from
    ...
    #[test]
    fn test_workspace_detect_prefers_dot_astralis_over_git() {
        let dir = tempfile::tempdir().unwrap();
        // Create both .astralis/ and .git
    ...
    fn test_workspace_id_adopts_existing() {
        ...
        std::fs::create_dir_all(ws.dot_astralis()).unwrap();
    ...
    fn test_workspace_path_accessors() {
        ...
        assert_eq!(
            ws.dot_astralis(),
            PathBuf::from("/home/user/project/.astralis")
        );
        assert_eq!(
            ws.workspace_id_path(),
            PathBuf::from("/home/user/project/.astralis/workspace-id")
        );
        assert_eq!(
            ws.instructions_path(),
            PathBuf::from("/home/user/project/.astralis/ASTRALIS.md")
        );
    }
}
