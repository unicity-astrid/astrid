...
//!
//! ```rust,ignore
//! use astralis_core::frontend::{Frontend, FrontendContext};
//!
//! struct MyFrontend;
//!
...
///
/// This trait defines the contract between the Astralis core and various
...
pub trait Frontend: Send + Sync {
    ...
    /// Resolve a frontend user ID to an Astralis identity.
    ///
    /// Returns `None` if the user is not known.
    async fn resolve_identity(&self, frontend_user_id: &str) -> Option<AstralisUserId>;
...
pub struct FrontendUser {
    ...
    /// Astralis identity (if resolved)
    pub astralis_id: Option<Uuid>,
    /// Display name
...
impl FrontendUser {
    ...
    /// Set the Astralis identity.
    #[must_use]
    pub fn with_astralis_id(mut self, id: Uuid) -> Self {
        self.astralis_id = Some(id);
        self
    }
