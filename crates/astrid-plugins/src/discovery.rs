...
//! the hook discovery pattern in `astrid-hooks`.
...
/// `AstridHome::plugins_dir()`) via `extra_paths` rather than relying
...
"#,
    ...
    fn test_workspace_plugins_dir() {
        ...
        assert_eq!(dir, PathBuf::from("/workspace/.astrid/plugins"));
    }
}
