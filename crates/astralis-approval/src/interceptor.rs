...
use astralis_capabilities::{CapabilityStore, CapabilityToken, ResourcePattern, TokenScope};
use astralis_core::error::{SecurityError, SecurityResult};
use astralis_core::types::{Permission, SessionId, TokenId};
...
impl SecurityInterceptor {
    ...
    fn handle_allow_always(&self, action: &SensitiveAction) -> SecurityResult<InterceptResult> {
        ...
        // AuditEntryId is the same type (re-exported from astralis_capabilities)
    ...
    fn create_allowance_for_action(
        ...
        let allowance = Allowance {
            ...
            created_at: astralis_core::types::Timestamp::now(),
            expires_at: None,
            max_uses: None,
            uses_remaining: None,
            session_only,
...
fn sensitive_action_to_audit(action: &SensitiveAction) -> AuditAction {
    match action {
        SensitiveAction::McpToolCall { server, tool } => AuditAction::McpToolCall {
            server: server.clone(),
            tool: tool.clone(),
            args_hash: astralis_crypto::ContentHash::zero(),
        },
        ...
        SensitiveAction::FileWriteOutsideSandbox { path } => AuditAction::FileWrite {
            path: path.clone(),
            content_hash: astralis_crypto::ContentHash::zero(),
        },
...
fn intercept_proof_to_audit(proof: &InterceptProof) -> AuditAuthProof {
    match proof {
        ...
        | InterceptProof::CapabilityCreated {
            ...
            token_hash: astralis_crypto::ContentHash::zero(),
        },
